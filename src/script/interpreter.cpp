...
static bool VerifyWitnessProgram(const CScriptWitness& witness, int witversion, const std::vector<unsigned char>& program, script_verify_flags flags, const BaseSignatureChecker& checker, ScriptError* serror, bool is_p2sh)
{
    ...
    if (witversion == 0) {
        ...
        // BIP-QP-ZIP: Post-quantum ZK-SNARK witness version 2 program
        // Legacy nodes treat this as ANYONECANSPEND (always valid) for soft-fork compatibility.
        // Upgraded nodes validate the post-quantum signature via the Rust FFI library.
        if (!(flags & SCRIPT_VERIFY_QPZIP)) {
            // If the node is not upgraded to support QP-ZIP, treat as ANYONECANSPEND (always valid)
            return set_success(serror);
        }
        return VerifyQPZipWitnessProgram(witness, program, checker, serror);
        ...
        // Other version/size/p2sh combinations return true for future soft-fork compatibility
        return true;
    }
