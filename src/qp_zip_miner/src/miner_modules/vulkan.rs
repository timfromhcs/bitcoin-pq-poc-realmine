...
impl VulkanEngine {
    ...
    fn init_vulkan(device_index: i32) -> Result<Self, String> {
        unsafe {
            ...
            let ac = std::ffi::CString::new("QP-ZIP Miner v2").unwrap();
            let ec = std::ffi::CString::new("GLM-5.2 MTP Engine").unwrap();
            let ai = ash::vk::ApplicationInfo::builder()
                .application_name(&ac).application_version(ash::vk::make_api_version(0,2,0,0))
                .engine_name(&ec).engine_version(ash::vk::make_api_version(0,5,2,0))
            ...
            // Improved VRAM detection for Ryzen 7000 iGPU (12GB shared memory)
            let mut vram = 0.0;
            ...
            // Integrated GPUs use shared memory - take total if VRAM looks low
